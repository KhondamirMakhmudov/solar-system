...
const MapOfUz = ({ color = "#6635BC", markersData = [] }) => {
  ...
  return (
    <div className="grid grid-cols-12 gap-4 w-full h-full">
          ...
          <g clipPath="url(#clip0_397_16207)">
            ...
            {markers.map((marker, index) => {
              const isSelected =
                ...
                (selectedMarker &&
                  !marker.id &&
                  markers.indexOf(marker) === markers.indexOf(selectedMarker));
              ...
              return (
                <g key={marker.id || index}>
                  ...
                  <circle
                    ...
                    r={isSelected ? "7" : "5"}
                    fill={isSelected ? "#8B5CF6" : "#FF0000"}
                    stroke="#FFFFFF"
                    strokeWidth="2"
                    className="cursor-pointer transition-all duration-200 manrope"
                    onMouseEnter={(e) => handleMarkerEnter(marker, e)}
                    onMouseLeave={handleMouseLeave}
                    onClick={(e) => handleMarkerClick(marker, e)}
                  />
                  {/* Animated pulse effect for selected marker */}
                  {isSelected && (
                    <circle
                      cx={marker.svgCoords.x}
                      cy={marker.svgCoords.y}
      ...
      {/* Details Panel */}
      {selectedMarker && (
        <div className="col-span-6 bg-[#1A132A] rounded-xl shadow-2xl overflow-hidden h-full manrope border border-[#2A1F3C]">
          {/* Header */}
          <div className="bg-gradient-to-r from-[#8B5CF6] to-[#7C3AED] p-6">
            <div className="flex justify-between items-start">
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-2">
                  <LocationOnIcon className="text-white" fontSize="medium" />
                  <h3 className="text-2xl font-bold text-white">
                    {selectedMarker.name}
                  </h3>
                </div>
                {selectedMarker.address && (
                  <p className="text-purple-100 text-sm ml-8">
                    {selectedMarker.address}
                  </p>
                )}
              </div>
          ...
          >
            ...
            <div className="bg-[#0F0A1E] rounded-lg p-4 border border-[#2A1F3C]">
              <h4 className="text-[#8B5CF6] text-sm font-semibold mb-3 uppercase tracking-wide">
                Coordinates
              </h4>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-[#A1A1AA] text-xs mb-1">Latitude</p>
                  <p className="text-[#E4E4E7] font-mono text-sm">
                    {selectedMarker.latitude?.toFixed(6)}
                  </p>
                </div>
                <div>
                  <p className="text-[#A1A1AA] text-xs mb-1">Longitude</p>
                  <p className="text-[#E4E4E7] font-mono text-sm">
            ...
            {Object.entries(selectedMarker).map(([key, value]) => {
              ...
              return (
                <div
                  key={key}
                  className="bg-[#0F0A1E] rounded-lg p-4 border border-[#2A1F3C] hover:border-[#8B5CF6] transition-all duration-200"
                >
                  <p className="text-[#8B5CF6] text-xs font-semibold mb-2 uppercase tracking-wide">
                    {key.replace(/_/g, " ")}
                  </p>
            ...
            {/* Empty State */}
            {Object.keys(selectedMarker).filter(
              (key) =>
                ![
                  "svgCoords",
                  "latitude",
              ...
              <div className="text-center py-8">
                <LocationOnIcon
                  className="text-[#2A1F3C] mx-auto mb-3"
                  style={{ fontSize: 48 }}
                />
